<div id="popup_detail_visit" class="popup_wrap" style="width: 600px;" v-cloak>
  <button type="button" class="popup_close" onclick="popup_detail_visit.sunrise('closePopup');">팝업 닫기</button>
  ...
  <div class="popup_contents">
    ...
    <div class="area">
      <div class="content_head">
        ...
        <button type="button" class="btn_update_book" v-if="!is_cancel" @click="popup_update_visit()">관리 정보 수정</button>
      </div>
      <div class="content_body">
        <table>
          <tbody>
            ...
            <tr v-if="is_cancel"> <!-- 0621 취소사유란 추가 -->
              <th>취소사유</th>
              <td><span class="desc cancle">{{ cancel_type }}</span>{{ cancel_reason }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="btn_area">
      <button type="button" class="btn e1 l" style="width: 220px;" onclick="popup_detail_visit.sunrise('closePopup');">닫기</button>
    </div>
  </div>
</div>
...
<script>
  ...
  var POPUP_DETAIL_VISIT = Vue.createApp({
    data() {
      return {
        res: POPUP_RES,
        req: {},
    ...
    methods: {
      popup_update_visit() {
        popup_update_visit = sunrise({
          target: '/schedule/popup_update_visit?id='+this.id+'&prev=client'
        })
      }
    }
  });

  POPUP_DETAIL_VISIT.mount('#popup_detail_visit');
</script>

            ...
            <tr>
              ...
              <td>
                <select style="width: 120px;" v-model="type_cd">
                  <option :value="1" v-if="remain_quantity>0">판매</option>
                  <option :value="2">입고</option>
                  <option :value="3" v-if="remain_quantity>0">소진</option>
                </select>
              </td>
  ...
  var POPUP_CHANGE_REGIST = Vue.createApp({
    data() {
      return {
        ...
        type_cd: (POPUP_RES.remain_quantity>0) ? 1 : 2,
        quantity: null,
        note: null,
        manager_id: null,
      }
    ...
    methods: {
      action_regist(type) {
        ...
        $.ajax({
          url: '/setting/cosmetic/action_regist_stock',
          data: req,
          success: (res) => {
            if (res.res_cd === 'OK') {
              location.href = `/setting/cosmetic/detail?id=`+this.cosmetic_id;
            } else {
              alert(res.err_msg);
            }
          }

<div id="popup_update" class="popup_wrap" style="width: 600px;" v-cloak>
  ...
  <div class="popup_head">
    <h1 class="popup_title">비품 정보 수정</h1>
  </div>
        ...
        <table>
          <tbody>
            <tr>
              ...
              <td>
                <input type="text" placeholder="비품명을 입력하세요" v-model="name">
              </td>
            </tr>
            <tr>
  ...
  var POPUP_UPDATE = Vue.createApp({
    ...
    methods: {
      action_regist() {
        ...
        $.ajax({
          url: '/setting/stuff/action_update',
          data: req,
          success: (res) => {
            if (res.res_cd === 'OK') {
              location.href = `/setting/stuff/detail?id=`+this.id;
            } else {
              alert(res.err_msg);
            }
          }
